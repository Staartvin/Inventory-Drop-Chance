...
public class Methods {
	...
	protected List<ItemStack> doSaveCheck(Player player,
		...
		if (plugin.wgClass.isWorldGuardReady()) {
			...
			calculated = drops.size() * (plugin.files.getRetainPercentage(player) / 100d);
		}
	...
	protected int calculateExp(int Exp, Player player) {
		...
		if (plugin.wgClass.isWorldGuardReady()) {
			...
			expLoss = (int) Math.round(Exp * (plugin.files.getExpPercentage(player) / 100d));
		}
		return Exp - expLoss;
	}
	...
	protected void returnItems(Player player, List<ItemStack> items) {
		...
		if (plugin.getConfig().getString("Groups." + plugin.files.getGroup(player) + ".check first").equalsIgnoreCase("save")) {
			percentageMessage = plugin.files.PERCENTAGE_MESSAGE_ON_RESPAWN;
		} else if (plugin.getConfig().getString("Groups." + plugin.files.getGroup(player) + ".check first").equalsIgnoreCase("delete")){
		...
		if (plugin.wgClass.isWorldGuardReady()) {
			...
			percentageMessage = percentageMessage.replace("{0}",
					plugin.files.getRetainPercentage(player) + "%").replace("{1}", plugin.files.getDeletePercentage(player) + "%");
			player.sendMessage(ChatColor.RED + percentageMessage);
		}
	}	
}
